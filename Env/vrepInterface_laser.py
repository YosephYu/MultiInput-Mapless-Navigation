...
class CarNavi(gym.Env):
    def __init__(self, path=None, device=None) -> None:
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(19,), dtype=np.float32)
        self.robotID = -1
    ...
    def reset(self):
        ...
        state = np.concatenate([ob, np.array([self.prev_dist/5, phi/np.pi, pose[2]/np.pi])])
        return state
    ...
    def step(self, action):
        ...
        state = np.concatenate([ob, np.array([dist/5, phi/np.pi, pose[2]/np.pi])])
