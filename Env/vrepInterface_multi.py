...
class CarNavi(gym.Env):
    def __init__(self, path=None, device=None) -> None:
        ...
        self.observation_space = spaces.Box(
            low=-1, high=1, shape=(51,), dtype=np.float32)
        self.robotID = -1
    ...
    def step(self, action):
        ...
        phi = np.arctan2(pose[1], pose[0])
        reward = self.get_reward(collision=collision, dist=dist, phi=phi, alpha=pose[2], min_dist=ob.min())
        ...
        rgbd = self.encoder(rgb, depth)
        state = np.concatenate([ob, np.array([dist/5, phi/np.pi, pose[2]/np.pi]), rgbd])
